...
#' @return A `mire_seq_range` or `mire_seq_ranges` object.
#' @examples
...
new_seq_range <- function(start, end) {
    structure(
        ...
        class = "mire_seq_range"
    )
}

is_range <- function(x) inherits(x, c("mire_seq_range", "mire_seq_ranges"))

is_seq_range <- function(x) inherits(x, "mire_seq_range")

is_seq_ranges <- function(x) inherits(x, "mire_seq_ranges")

#' @export
c.mire_seq_range <- function(...) {
    ...
    if (length(ranges) == 1L) {
        structure(.subset2(ranges, 1L), class = "mire_seq_range")
    } else {
        structure(ranges, class = "mire_seq_ranges")
    }
}

#' @export
c.mire_seq_ranges <- c.mire_seq_range

#' @export
print.mire_seq_range <- function(x, ...) {
    cat("<seq_range>", "\n", sep = "")
    cat(
        .subset2(x, "start") %||% " ",
        .subset2(x, "end") %||% " ",
...
#' @export
print.mire_seq_ranges <- function(x, ...) {
