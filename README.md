...
``` r
odir <- "scmire" # Specify your output directory
...
# to transform the `umi` into fq2 and then run `scmire` with only fq2.
fq1 <- # Sequence file
...
# sahmi_datasets <- lapply(paths, scmire::read_dataset)
```

### Identify contaminants

``` r
library(polars) # scmire dependents on `polars`
library(ggplot2)
truly_microbe <- scmire::remove_contaminants(
...
``` r
slsd <- scmire::slsd(lapply(sahmi_datasets, `[[`, "kreport"))
...
``` r
counts <- scmire::taxa_counts(umi_list, basename(names(umi_list)))
